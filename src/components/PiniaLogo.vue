<template>
  <svg
    ...
    :class="[autoFixed ? 'pinia-position' : '', customClass || '']"
    :width="width"
    :height="height"
    ...
    @mouseenter="handerEnter"
    @mouseleave="handerLeave"
  >
    <g class="leaves">
      <path
        fill-rule="evenodd"
...
const props = defineProps({
  ...
  play: {
    type: String as PropType<'LeftClick' | 'Hover' | 'None'>,
    default: 'None'
  },
  penetrate: {
    type: Boolean,
...
watch(isEnter, (newVal, oldVal) => {
  if (oldVal === false && play.value === 'Hover') {
    withBlink()
    withTalk()
  }
})
...
.leaves {
  ...
  animation: 5s ease-in-out 5s infinite normal leaves-move;
